...
export function DifficultyVsPairsChart({ data }) {
  if (!data || data.length === 0) {
    return (
      <div className="bg-gray-800 rounded-lg p-6">
        <h2 className="text-lg font-semibold mb-4">Difficulty vs Pairs Planned</h2>
  ...
  // Filter out sessions without avg_difficulty
  const validData = data.filter(session => session.avg_difficulty != null);
  ...
  if (validData.length === 0) {
    return (
      <div className="bg-gray-800 rounded-lg p-6">
        <h2 className="text-lg font-semibold mb-4">Difficulty vs Pairs Planned</h2>
        <div className="h-64 flex items-center justify-center text-gray-500">
          No session data with difficulty information
        </div>
      </div>
    );
  }

  const chartData = validData.map((session) => {
    ...
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-lg p-3 max-w-xs">
        ...
        <div className="space-y-1 text-sm">
          <p className="text-blue-400">Avg Difficulty: {d.avgDifficulty.toFixed(1)} moves</p>
  ...
  const minDifficulty = Math.min(...chartData.map(d => d.avgDifficulty));
  const maxDifficulty = Math.max(...chartData.map(d => d.avgDifficulty));

  return (
    <div className="bg-gray-800 rounded-lg p-6">
      <h2 className="text-lg font-semibold mb-2">Difficulty vs Pairs Planned</h2>
      <p className="text-sm text-gray-400 mb-4">
        How pairs planned varies with scramble difficulty
      </p>
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <XAxis
              ...
              stroke="#9CA3AF"
              fontSize={12}
              domain={[Math.floor(minDifficulty) - 0.5, Math.ceil(maxDifficulty) + 0.5]}
              label={{ value: 'Avg Difficulty (moves)', position: 'bottom', fill: '#9CA3AF', fontSize: 12, offset: 0 }}
            />
            <YAxis
              type="number"
              dataKey="avgPairs"
